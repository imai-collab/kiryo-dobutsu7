                    ...
                    <button
                      onClick={async () => {
                        try {
                          ...
                          alert("最新のデータをアプリ本体（puzzles.json）に保存しました！\nAI Studioの「Sync to GitHub」からGitHubへ反映してください。");
                        } catch (err) {
                          alert("保存に失敗しました。");
                          console.error(err);
                        }
                      }}
                      className="text-xs px-3 py-1 font-bold rounded-lg border-2 bg-[#E8F5E9] border-[#4CAF50] text-[#2E7D32] hover:bg-[#C8E6C9] transition-all"
                    >
                      アプリに保存(Sync用)
                    </button>
                    <input
